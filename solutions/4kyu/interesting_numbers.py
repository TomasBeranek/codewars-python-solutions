...
# there's an interesting number coming up, he notices and then promptly forgets.
...
def num_is_interesting(n, awesome_phrases):
    s = str(n)

    if set(s[1:]) == set('0') or \
       len(set(s)) == 1 or \
       ''.join(map(str, list(range(int(s[0]), int(s[-1])+1)))) == s or \
       (''.join(map(str, list(range(int(s[0]), int(s[-2])+1)) + ['0'])) == s and s[-2] == '9') or \
       ''.join(map(str, list(range(int(s[0]), int(s[-1])-1, -1)))) == s or \
       (''.join(map(str, list(range(int(s[0]), int(s[-2])-1, -1)) + ['0'])) == s and s[-2] == '1') or \
       s[::-1] == s or \
...
def is_interesting(number, awesome_phrases):
    if num_is_interesting(number, awesome_phrases):
        return 2
    elif num_is_interesting(number+1, awesome_phrases) or num_is_interesting(number+2, awesome_phrases):
        return 1
    else:
        return 0
