...
def recursive_brute_force(num_to_try, observed):
    possible_variations = {'1': ['1', '2', '4'],
                           '2': ['1', '2', '3', '5'],
                           '3': ['2', '3', '6'],
                           '4': ['1', '4', '5', '7'],
                           '5': ['2', '4', '5', '6', '8'],
    ...
    if not observed:
        return possible_variations[num_to_try]

    pins = recursive_brute_force(observed[0], observed[1:])
    extended_pins = []
    for digit in possible_variations[num_to_try]:
        for pin in pins:
            extended_pins.append(digit + pin)

    return extended_pins


def get_pins(observed):
    return recursive_brute_force(observed[0], observed[1:])
